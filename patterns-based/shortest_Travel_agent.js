 ...
 * Custom test casesformat: [7, 3, 7, 3, 1, 3, 4, 1]
 */


function ShorestVacations(arr) {
    let uniqArr = new Set(arr);
    let minVacationSize = uniqArr.size;
    let vacationSize=minVacationSize;
    let found = false;

    while (!found) {
        let start=0;
        for (let i = 0; i + vacationSize < arr.length; i++) {
            let resultArr = arr.slice(start, (start+vacationSize));
            let resultSet = new Set(resultArr);
            if (resultSet.size === minVacationSize) {
                found = true;
...
ShorestVacations([7, 3, 7, 3, 1, 3, 4, 1]);
